...
if (!GREN_GITHUB_TOKEN) {
  console.error(
    chalk.red("Can't find GREN_GITHUB_TOKEN. Please configure your environment") +
      chalk.blue("\nSee https://github.com/cjbarth/github-release-notes#setup"),
  );
  process.exit(1);
}
...
const getLabels = async () => {
  ...
  try {
    const octokit = new Octokit({ auth: GREN_GITHUB_TOKEN });
    const { data: labels } = await octokit.rest.issues.listLabelsForRepo({ owner: username, repo });
    return labels;
  } catch {
    console.warn(
      chalk.bgYellow(
