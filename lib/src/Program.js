...
class Program {
  constructor(props) {
    ...
    // Commander keeps the parsed options on opts(). Reading one off the command itself gives
    // undefined without complaining, which here meant --config was ignored and the changelog
    // was built from whichever .grenrc happened to be in the directory.
    const opts = this.program.opts();

    this.options = Object.assign(
      {},
      getGrenConfig(props.cwd, opts.config),
      this._getOptionsFromObject(opts, this.defaults),
    );
  }
