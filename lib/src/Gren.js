...
class Gren {
  constructor(props = {}) {
    ...
    try {
      ...
      this.options.head = execSync("git remote show origin | grep 'HEAD branch' | cut -d' ' -f5", {
        encoding: "utf-8",
      });
    }
  ...
  async _getMergedPullRequests(since, page = 1, limit = 1000) {
    ...
    if (
      prs.length > 0 &&
      new Date(since) < new Date(prs[prs.length - 1].updated_at) &&
      totalPages &&
      +page < totalPages
    ) {
      return this._getMergedPullRequests(since, page + 1).then((prsResults) =>
        prsResults.concat(filterPrs),
      );
    }
  ...
  _filterBlockPullRequest(range, pullRequest) {
    if (this.options.dataSource === "milestones") {
      return (
        this.options.milestoneMatch.replace("{{tag_name}}", range[0].name) ===
        pullRequest.milestone.title
      );
    }

    return range[2].some((commit) => pullRequest.merge_commit_sha === commit.sha);
  }
  ...
  async _getPullRequestsBlocks(releaseRanges) {
    ...
    const filteredPrs = prs.filter(this._filterPullRequest.bind(this));

    let totalPrs = 0;
    const release = releaseRanges.map((range) => {
      const filteredPullRequests = filteredPrs.filter(
  ...
  async _getPullRequestWithIssueBlocks(releaseRanges) {
    ...
    const filteredPrs = prs.filter(this._filterPullRequest.bind(this)).map((pr) => {
    ...
    const release = releaseRanges.map((range) => {
      const list = filteredPrs.filter(this._filterBlockPullRequest.bind(this, range));
  ...
  _sortReleasesByDate(releaseDates) {
    return Array.from(releaseDates).sort((release1, release2) => {
      if (release1.date == null || release2.date == null) {
        // compare by semver
        return cmp(release2.name, release1.name);
      } else {
  ...
  async _getReleaseBlocks() {
    ...
    for (const releaseRange of releaseRanges) {
      ...
      if (releaseRange[1].name != null) {
        let response;
        try {
          response = await this.repo.compareBranches(releaseRange[1].name, releaseRange[0].name);
        } catch {
          // If the above failed, it is because we are using a calculated version as the first `name`
          // Use the head of the branch instead
          response = await this.repo.compareBranches(releaseRange[1].name, this.options.head);
        }
        ({
          data: { commits },
        } = response);
  ...
  _sortCommitsByParent(commits, dateOrder = "desc") {
    ...
    // 2) Compute in‑list parents for each commit
    const parentMap = new Map();
    for (const c of commits) {
      const inList = (c.parents || []).map((p) => p.sha).filter((sha) => commitMap.has(sha));
      parentMap.set(c.sha, inList);
    }

    // 3) Compute which SHAs appear as a parent → those that have children
    const hasChild = new Set();
    for (const parents of parentMap.values()) {
      for (const p of parents) hasChild.add(p);
    }
    ...
    while (pool.length) {
      ...
      // add its in‑list parents to the pool
  ...
  _outputOptions(options) {
    ...
    const outputs = Object.entries(options)
      ...
      .map(
        ([key, value]) => `${chalk.yellow(camelcaseToSpaces(key))}: ${value.toString() || "empty"}`,
      );
