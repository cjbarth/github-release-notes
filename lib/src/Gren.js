...
class Gren {
  ...
  _selectMembershipReleases(tags) {
    ...
    if (head && unreleasedVersion && !tags.some((tag) => tag.version === unreleasedVersion)) {
      if (!git.refExists(head)) {
        throw chalk.red(`\nThe branch "${head}" is not in the local repository.`);
      }

      const merged = git.tagsMergedInto(head);
      const newer = tags.find(
        (tag) => merged.has(tag.name) && semver.gte(tag.version, unreleasedVersion),
      );

      if (newer) {
        console.warn(
          chalk.yellow(
            `\nSkipping the unreleased ${version} section: ${head} already contains ${newer.name}.`,
          ),
        );
      } else {
        releases.push({
