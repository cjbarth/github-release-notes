...
class Gren {
    ...
    async _getCommitBlocks(releaseRanges) {
        ...
        const ranges = releaseRanges
            .map(range => ({
                id: range[0].id,
                name: this.options.prefix + range[0].name,
                release: range[0].name,
                published_at: range[0].date,
                body: this._generateCommitsBody(range[1].commits, range[1].id === 0) + '\n'
            }));
    ...
    _filterBlockPullRequest(range, pullRequest) {
        ...
        return range[1].commits.some(commit => (pullRequest.merge_commit_sha === commit.sha));
    }
    ...
    async _getPullRequestsBlocks(releaseRanges) {
        ...
        const release = releaseRanges
            .map(range => {
                const filteredPullRequests = filteredPrs
    ...
    async _createReleaseRanges(releaseDates) {
        ...
        const loaded = utils.task(this, 'Getting the list of commits by release');
        for (let i = 0; i < sortedReleaseDates.length - RANGE + 1; i++) {
            const until = sortedReleaseDates[i + 1].date;
            const previousTag = sortedReleaseDates[ i + 1 ].name;
            const nextTag = sortedReleaseDates[i].name;
            let commits;
            if (previousTag != null) {
                ({data: { commits }} = await this.repo.compareBranches(previousTag, nextTag));
            } else {
                ({data: commits} = await this.repo.listCommits({ until }));
            }
            ranges.push([
