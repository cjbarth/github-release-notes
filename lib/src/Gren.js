...
// The heading for commits that no release contains yet.
  ...
  async _getMembershipReleaseBlocks(getBlocks, existingSections) {
    ...
    return entries
      .map(({ release, frozen, verbatim }) => {
        ...
        // There is no version for the prefix to belong to in an unreleased section.
        return release.version ? block : { ...block, name: release.name };
      })
      .filter(Boolean);
  }
        ...
        console.warn(
          chalk.yellow(
            `\nThe version being prepared, ${version}, is older than ${behind.name}, which ` +
              `${head} already contains. Commits no tag holds are listed as ${UNRELEASED}.`,
          ),
        );
      }
