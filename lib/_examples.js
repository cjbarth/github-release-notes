...
export function generateExamples(title, examples) {
  process.stdout.write(`\n  Examples for ${title}:`);
  const spaceify = (number) => new Array(!number || number < 0 ? 0 : number + 1).join(" ");
  const spaces = spaceify(6);
  const descriptionPlaceholder = spaceify(
    Math.max(...examples.map(({ description }) => (description ? description.length : 0))),
  );
...
export const release = [
  ...
  },
  {
    ...
    description:
      'Create release notes for a tag using the belonging to a milestone that matches the name of the tag. e.g. If the tag is 4.0.0, `gren` is going to match the milestone _"Release 4.0.0"_.',
