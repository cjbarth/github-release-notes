...
describe("Gren", () => {
  ...
  describe("Issues and PRs", () => {
    ...
    describe("_getLastPage", () => {
      ...
      it("Should return false", () => {
        ...
        assert.isNotOk(
          gren._getLastPage("Lorem ipsum"),
          "The string does not contain the page information",
        );
    ...
    describe("_groupBy, _groupByLabel", () => {
      ...
      it("Should return the group based on issue labels", () => {
        ...
        assert.deepEqual(
          gren._groupBy(normal),
          [`${normal[0].labels[0].name}\n${normal[0].title}`],
          'Group option is "label"',
        );
        assert.deepEqual(
          gren._groupBy(noLabel),
          [`closed\n${noLabel[0].title}`],
          'Group option is "label" with no labels',
        );
      });
      ...
      it("Should not return labels without labels", () => {
        ...
        assert.lengthOf(
          gren._groupBy(noLabel),
          0,
          "When the noLabel is false and only an issue with no labels has been passed",
        );
        assert.lengthOf(
          gren._groupBy(noLabel.concat(normal)),
          1,
          "When the noLabel is false and only one issue with labels has been passed",
        );
      });
      ...
      it("Should group the issues based on the option", () => {
        ...
        assert.deepEqual(
          gren._groupBy(normal),
          [`Test\n${normal[0].title}`],
          "Passing one heading for one issue",
        );
        assert.deepEqual(
          gren._groupBy(noLabel),
          [`Others\n${noLabel[0].title}`],
        ...
        assert.deepEqual(
          gren._groupBy(normal),
          [],
          "Passing one heading that does not match any labels",
        );
        ...
        assert.deepEqual(
          gren._groupBy(normal),
          [`All\n${normal[0].title}`],
          "The issue does not match any labels, and goes in the ... group",
        );
      });
      ...
      it("Should format group using post formatter", () => {
        ...
        gren.options.groupPostProcessor = (groupContent) =>
          groupContent.replace(0, groupContent.indexOf(":\n") + 3);

        assert.deepEqual(
          gren._groupBy(normal),
          ["Test:\nIssue with one label and milestone"],
          "Passing one heading for one issue",
        );
        assert.deepEqual(
          gren._groupBy(noLabel),
          ["Others:\nIssue with no milestone and no labels"],
          'Group option is "label" with no labels',
        );
      });
    });
    ...
    describe("_filterIssue", () => {
      it("Should not return the wrong issue", () => {
        ...
        assert.isNotOk(
          gren._filterIssue(noMilestone[0]),
          "Issue without milestone, with onlyMilestone as true",
        );
        ...
        assert.isNotOk(
          gren._filterIssue(noMilestone[0]),
          "Issue without milestone, with dataSource as milestone",
        );

        gren.options.dataSource = "prs";
        assert.isNotOk(
          gren._filterIssue(normal[0]),
          "Issue are not included, with dataSource as prs",
        );
      });
      ...
      it("Should include pull requests", () => {
        gren.options.dataSource = "prs";
        assert.isOk(
          gren._filterIssue(issues.pullRequests[0]),
          "Pull Requests are included, with dataSource as prs",
        );
      });
    });
    ...
    describe("_lablesAreIgnored", () => {
      it("Should return false on unsupported paramater", () => {
        ...
        assert.isNotOk(
          gren._lablesAreIgnored([1, 2, 3]),
          "The variable is a Array of invalid data",
        );
      });
      ...
      it("Should return false if none of the label names are included", () => {
        ...
        assert.isNotOk(
          gren._lablesAreIgnored([{ name: "a" }, { name: "b" }]),
          "None of the names match",
        );
        assert.isNotOk(
          gren._lablesAreIgnored([{ name: "ac" }, { name: "b" }]),
          "Part of the name is in a label name",
        );
      });
      ...
      it("Should return true if it finds any label name", () => {
        ...
        assert.isOk(
          gren._lablesAreIgnored([{ name: "c" }, { name: "e" }]),
          "Only one of the labels match",
        );
        assert.isOk(
          gren._lablesAreIgnored([{ name: "c" }, 1]),
          "Only one of the labels match and is valid data",
        );
      });
    });
  });
  ...
  describe("_generateCommitsBody, _templateCommits, _filterCommit", () => {
    ...
    it("Should only return the messages defined in the options", () => {
      ...
      const messages = (msg) =>
        `${commitMessages[msg].commit.message} - ${commitMessages[msg].author.login}`;

      assert.deepEqual(
        gren._generateCommitsBody(commitMessages),
        `${messages(0)}\n${messages(1)}\n${messages(3)}`,
        "Using commits as includeMessages",
      );
    ...
    it("Should not return commits with ignored words", () => {
      ...
      assert.notInclude(
        commitMessages.filter((message) => gren._filterCommit(message)),
        messages(1),
        "Ignore another",
      );
    });
  ...
  describe("_createChangelog", () => {
    ...
    it("Should create the changelog file", () => {
      ...
      assert.deepEqual(
        fs.readFileSync(gren.options.changelogFilename, "utf-8"),
        gren.options.template.changelogTitle + changelogBody,
        "Add the right body to the changelog file",
      );
    });
  ...
  describe("_transformTagsIntoReleaseObjects", () => {
    it("Should transform tags into release objects", () => {
      const tag = {
        tag: { name: "tagName", number: 3 },
        releaseId: "12315",
        date: "2020-02-20T13:40:16Z",
      };
  ...
  describe("_validateRequiredTagsExists", () => {
    it("should failed if one tag is missing", () => {
      ...
      assert.throw(
        () => gren._validateRequiredTagsExists([existingTag], ["some_tag", "existing_tag"]),
        expectedException,
      );
    });
    ...
    it("should failed if the two input tags are missing", () => {
      ...
      assert.throw(
        () => gren._validateRequiredTagsExists([], ["some_tag", "some_other_tag"]),
        expectedException,
      );
    });
            ...
            gren
              ...
              .then((tags) => {
                assert.notInclude(
                  tags.map(({ name }) => name),
                  "0.16.0",
                  "The ignored tag is not present",
                );
                done();
              })
              .catch((err) => done(err));
