...
describe("Program", () => {
  ...
  describe("_dashToCamelCase", () => {
    ...
    it("Should return a camelCase string", () => {
      assert.deepEqual(
        testCommand._dashToCamelCase("i-am-test"),
        "iAmTest",
        "Pass a dasharised string",
      );
      assert.deepEqual(
        testCommand._dashToCamelCase("i-amTest"),
        "iAmTest",
        "Pass a partally dasharised string",
      );
    });
  });
  ...
  describe("_camelCaseObjectKeys", () => {
    ...
    it("Should convert in camelCase all the keys of an Object", () => {
      assert.hasAllKeys(
        testCommand._camelCaseObjectKeys({
          "a-normal-string": 1,
          "another-string": 2,
          "someThing-else": 3,
        }),
        ["aNormalString", "anotherString", "someThingElse"],
        "Passing an Object with mixed values",
      );
    });
  });
  ...
  describe("_getOptionsFromObject", () => {
    ...
    it("Should only take the options from defaults", () => {
      assert.deepEqual(
        testCommand._getOptionsFromObject(
          {
            a: 1,
            b: 2,
            c: 3,
        ...
        { a: 1, c: 3 },
        "Passing Object which contains few keys",
      );
      ...
      assert.deepEqual(
        ...
        { a: 1, b: 2 },
        "Object contains more keys than object value",
      );
    });
  });
  ...
  describe("_consumeOptions", () => {
    ...
    it("Should return an Object", () => {
      ...
      assert.hasAllKeys(
        testCommand._consumeOptions(options),
  ...
  describe("_filterObject", () => {
    it("Should return an object without undefined values", () => {
      assert.deepEqual(
        testCommand._filterObject({ a: 1, b: undefined, c: false }),
        { a: 1, c: false },
        "Pass with values number, undefined and false",
      );
    });
  });
});
