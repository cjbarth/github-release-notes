...
describe("_options.js", () => {
  it("Should have unique shorts", () => {
    const duplicates = globalOptions
      .concat(releaseOptions, changelogOptions)
      .map(({ short }) => short)
      .filter(Boolean)
      .filter((short, index, array) => array.indexOf(short) !== index).length;
  ...
  it("Should work with ignore-labels", () => {
    ...
    assert.deepEqual(
      JSON.stringify(action("1,2")),
      JSON.stringify(["1", "2"]),
      "Testing the option ignore-labels",
    );
  });
  ...
  it("Should work with ignore-issues-with", () => {
    ...
    assert.deepEqual(
      JSON.stringify(action("1,2")),
      JSON.stringify(["1", "2"]),
      "Testing the option ignore-issues-with",
    );
  });
});
